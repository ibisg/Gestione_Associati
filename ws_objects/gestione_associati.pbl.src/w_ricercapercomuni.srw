$PBExportHeader$w_ricercapercomuni.srw
forward
global type w_ricercapercomuni from window
end type
type lb_filtro from listbox within w_ricercapercomuni
end type
type pb_dom from picturebutton within w_ricercapercomuni
end type
type st_3 from statictext within w_ricercapercomuni
end type
type ddlb_localita from dropdownlistbox within w_ricercapercomuni
end type
type sle_filtro from singlelineedit within w_ricercapercomuni
end type
type pb_ordina from picturebutton within w_ricercapercomuni
end type
type sle_struttura from singlelineedit within w_ricercapercomuni
end type
type st_2 from statictext within w_ricercapercomuni
end type
type st_1 from statictext within w_ricercapercomuni
end type
type pb_go from picturebutton within w_ricercapercomuni
end type
type cbx_resp from checkbox within w_ricercapercomuni
end type
type ddlb_provincia from dropdownlistbox within w_ricercapercomuni
end type
type ddlb_regione from dropdownlistbox within w_ricercapercomuni
end type
type dw_current from datawindow within w_ricercapercomuni
end type
type gb_filtro from groupbox within w_ricercapercomuni
end type
type gb_criteri from groupbox within w_ricercapercomuni
end type
end forward
...
global w_ricercapercomuni w_ricercapercomuni
...
forward prototypes
public function integer wf_rif ()
end prototypes
...
s_preview_s.ls_addsqlsyntax= ls_sqlsyntax
s_preview_s.ib_titolografico= false

openWithParm(w_preview, s_preview_s)

w_preview.uodw_current.retrieve()
...
event ue_layout();setpointer(hourGlass!)

if lower(dw_current.Object.DataWindow.Print.Preview)= 'yes' then
...
public function integer wf_rif ();string ls_codice
s_ricercanominativa s_ricercanominativa_s

//if row <= 0 then return

//ls_codice= this.getItemString(row, "codice")
s_ricercanominativa_s.codice=  	dw_current.getItemString(dw_current.getRow(), "codice")
s_ricercanominativa_s.cognome= dw_current.getItemString(dw_current.getRow(), "cognome")
s_ricercanominativa_s.nome=  	dw_current.getItemString(dw_current.getRow(), "nome")


return openWithParm(w_mostra_riferimenti, s_ricercanominativa_s)
end function
...
on w_ricercapercomuni.create
this.lb_filtro=create lb_filtro
this.pb_dom=create pb_dom
...
on w_ricercapercomuni.destroy
destroy(this.lb_filtro)
destroy(this.pb_dom)
destroy(this.st_3)
...
fontfamily fontfamily = swiss!
string facename = "Tahoma"
string text = "Applica filtro"
boolean default = true
boolean originalsize = true
vtextalign vtextalign = multiline!
string powertiptext = "Applica il filtro"
...
string powertiptext = "Cliccare questo tasto per modificare l~'ordine dei risultati"
end type
...
string text = "Verranno estratti solo i membri partecipanti alle attività"
...
alignment htextalign = left!
string powertiptext = "Cliccare questo tasto per eseguire la ricerca"
end type
...
is_provincia= ""
is_localita= ""
sle_struttura.text= "Verranno estratti solo i membri partecipanti alle attività"
...
event retrievestart;pb_ordina.enabled= false
end event

event buttonclicked;wf_rif()
end event
