$PBExportHeader$w_liste_invio_tab_old.srw
forward
global type w_liste_invio_tab_old from window
end type
type tab_send from tab within w_liste_invio_tab_old
end type
...
type st_sms from statictext within tabpage_sms
end type
type sle_sms from singlelineedit within tabpage_sms
end type
...
type gb_2 from groupbox within w_liste_invio_tab_old
end type
type gb_5 from groupbox within w_liste_invio_tab_old
end type
type gb_6 from groupbox within w_liste_invio_tab_old
end type
type gb_7 from groupbox within w_liste_invio_tab_old
end type
end forward
...
global w_liste_invio_tab_old w_liste_invio_tab_old
...
if trap_sql(sqlca, "LISTA01") < 0 then return -1
...
choose case as_lista_s.tipo_lista
	...
	case "MAIL"
		...
		ll_count= tab_send.tabpage_mail.dw_liste_mail.retrieve(as_lista_s.idx, as_lista_s.cod_rif)
		 ...
		 tab_send.tabpage_mail.dw_liste_mail.setFilter ("match( riferimento_descr , '@@') or (NOT match(  riferimento_descr , '@')) or isNull(riferimento_descr)" )
	...
	case "SMS"
		...
		ll_count= tab_send.tabpage_sms.dw_liste_sms.retrieve(as_lista_s.idx, as_lista_s.cod_rif)
		 ...
		 tab_send.tabpage_sms.dw_liste_sms.setFilter("riferimento_descr= '0' or isNull(riferimento_descr)" )
		 tab_send.tabpage_sms.dw_liste_sms.filter()
			for ll_idx= 1 to ll_count
				 tab_send.tabpage_sms.dw_liste_sms.setItem(ll_idx, "invio_ok", "0")
			next
		 tab_send.tabpage_sms.dw_liste_sms.setFilter("")
		 tab_send.tabpage_sms.dw_liste_sms.filter()			
		
end choose

if ll_count > 0   then tab_send.enabled= true
...
if messageBox(this.title, "Stai scrivendo un messaggio: se prosegui verranno perse le modifiche. ~nVuoi proseguire?", question!, yesNo!, 2) = 2 then return 1 // non prosegue l'elaborazione
...
on w_liste_invio_tab_old.create
...
on w_liste_invio_tab_old.destroy
...
//idwc_mailrif.setTransObject(sqlca)
//tab_send.tabpage_mail.dw_liste_mail.getChild("riferimento_descr", idwc_mailrif)
...
//choose case tab_send.selectedTab
//		
//	case 1
//		
...
//	
//		tab_send.tabpage_mail.mle_messaggio.height= tab_send.tabpage_mail.gb_body.height  - 20
//		
//		tab_send.tabpage_mail.dw_allegati.height= tab_send.tabpage_mail.gb_allegati.height  - 20
//		
//	case 2
//		
...
//	tab_send.tabpage_etichette.dw_etichette.height= tab_send.tabpage_etichette.gb_labels.height - tab_send.tabpage_etichette.dw_etichette.y - 30
//
//case 4
//	
//	tab_send.tabpage_sms.gb_bodysms.height= tab_send.height - tab_send.tabpage_sms.gb_bodysms.y - 30
////
////	tab_send.tabpage_sms.mle_messaggio.height= tab_send.tabpage_sms.gb_body.height  - 240
//
//end choose
//
end event

event closequery;return wf_ismessage() 	
end event
...
long ll_idx= 1, ll_tgt= 1, ll_found, ll_totrows
...
if pos(mle_messaggio.text, '"') > 0 then
	...
	messageBox(parent.text, "Rimuovere le virgolette dal testo del messaggio", information!)
	return 0
...
if sle_oggetto.text= "" or isNull(sle_oggetto.text) then
	...
	messageBox(parent.text, "Occorre inserire l'oggetto del messaggio", information!)
	return 0
...
if pos(sle_oggetto.text, '"') > 0 then
	...
	messageBox(parent.text, "Rimuovere le virgolette dall'oggetto", information!)
	return 0
...
if sle_mittente.text= "" or isNull(sle_mittente.text) or pos(sle_mittente.text , "@")= 0 then
	...
	messageBox(parent.text, "Indirizzo mail non corretto", information!)
	return 0
...
if pos(sle_mittente.text, " ") <> 0 then
	...
	messageBox(parent.text, "L'indirizzo del mittente contiene uno spazio.~nRimuoverlo e poi ritentare l'invio del messaggio.", information!)
	return 0
...
if ls_disclaimer= "" then messageBox("Disclaimer", "Errore in lettura disclaimer") 

lmailMessage_s.noteText += ls_disclaimer

open(w_wait_snoopy_mail)
...
li_random= rand(30000)
	...
	for ll_idx= 1 to dw_liste_mail.rowCount()
		...
		if dw_liste_mail.getItemString(ll_idx, "invio_ok")= '0' then continue
				...
				li_tipo= integer(dw_liste_mail.getItemString(ll_idx, "cmodalitainvio"))				
	...
	select distinct riferimento_descr, tipo_invio
	from liste_tmp
	...
	do while sqlca.sqlcode <> 100
		...
		fetch liste_cur into :ls_descr, :li_tipo;
...
// li_ret= 1 = NOK, li_ret= 0 = OK
li_ret=  f_sendmail_blat(lmailMessage_s, ls_sms_s)

close(w_wait_snoopy_mail)
...
event clicked;ib_receiptrequested= cbx_receipt.checked
end event
...
choose case dwo.name
	...
	case "b_modifyrif"
		...
		string ls_message, ls_where, ls_
		...
		// può modificare il riferimento corrente
				...
				s_riferimenti_s.cod_rif= is_lista_s.cod_rif
			
				s_riferimenti_s.idx= this.getItemDecimal(row, "idx")
				
				
				if 	s_riferimenti_s.idx= 0 or isNull(	s_riferimenti_s.idx) then

					if messageBox(parent.text, "Vuoi inserire il riferimento?", Question!, yesNo!, 2)= 2 then return 0
					
				else
					
					if messageBox(parent.text, "Vuoi modificare il riferimento corrente?", Question!, yesNo!, 2)= 2 then return 0
				...
				event ue_list(is_lista_s)
				
				this.scrollToRow(row)
	
//				ls_where= "  WHERE riferimenti.predefinito= 1 and riferimenti.cod_rif = ~~'MA~~'    and riferimenti.codice= ~~'"+s_riferimenti_s.codice+"~~'"
//			
//				tab_send.tabpage_mail.dw_liste_mail.getChild("riferimento_descr", idwc_mailrif)
...
event itemfocuschanged;if dwo.name= "riferimento_descr" and row > 0 then 
	long ll_rows
	string ls_where
	
	is_codice= this.getItemString(row, "codice")
//	ls_where= "  WHERE riferimenti.predefinito= 1 and riferimenti.cod_rif = ~~'MA~~'    and riferimenti.codice= ~~'"+is_codice+"~~'"
	ls_where= "  WHERE riferimenti.cod_rif = ~~'MA~~'    and riferimenti.codice= ~~'"+is_codice+"~~'"

	tab_send.tabpage_mail.dw_liste_mail.getChild("riferimento_descr", idwc_mailrif)
	ll_rows= f_mod_select(ls_where, idwc_mailrif)
end if
end event
...
string facename = "Arial"
long backcolor = 67108864
string text = "Indirizzo del mittente"
end type
...
string ls_message, ls_destinatario
...
if len(mle_messaggio.text) <> 0 then ls_message= mle_messaggio.text
...
for ll_idx= 1 to dw_liste_mail.rowCount()
	...
	ls_destinatario +=gc_crlf+ f_iniziali(trim(dw_liste_mail.getItemString(ll_idx, "nome") )+" "+trim(dw_liste_mail.getItemString(ll_idx, "cognome")))
	
next

mle_messaggio.text= ""
mle_messaggio.text="Lista di distribuzione:"+ls_destinatario+gc_crlf+ls_message
...
this.st_sms=create st_sms
this.sle_sms=create sle_sms
...
string text = "Restanti caratteri:"
boolean focusrectangle = false
end type
...
event itemfocuschanged;if dwo.name= "riferimento_descr" and row > 0 then
	...
	is_codice= this.getItemString(row, "codice")
	ls_where= "  WHERE riferimenti.cod_rif = ~~'TM~~'    and riferimenti.codice= ~~'"+is_codice+"~~'"

	tab_send.tabpage_sms.dw_liste_sms.getChild("riferimento_descr", idwc_mailrif)
	f_mod_select(ls_where, idwc_mailrif)
end if
end event

event retrieveend;if rowcount> 0 then tab_send.tabpage_sms.cb_inviasms.enabled= true
...
event other;sle_sms.text= string( 160 - len(this.text) )
end event
