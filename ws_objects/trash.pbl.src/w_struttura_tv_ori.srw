$PBExportHeader$w_struttura_tv_ori.srw
forward
global type w_struttura_tv_ori from window
end type
type lv_dettaglio from listview within w_struttura_tv_ori
end type
type tv_struttura from treeview within w_struttura_tv_ori
end type
end forward
...
event type integer ue_queryofferte ( integer ai_qualequery,  long al_livello,  string as_nodo,  s_data as_data_s )
event type integer ue_cerca ( )
lv_dettaglio lv_dettaglio
tv_struttura tv_struttura
end type
global w_struttura_tv_ori w_struttura_tv_ori
...
public function integer lvf_dettaglio ()
...
choose case ai_qualequery
	case 1
		ls_object= 'dw_queryofferte_grid'
	case 2
...
if ai_source= 1 then // la query viene richiesta dal treeView...
	...
	openWithParm(w_selezione_livello, s_struttura_loc.livello)
...
w_preview_scheda.setRedraw(false)
	...
	ls_query= "select vc_nodo from struttura where livello="+string(li_livello_raggruppamento)+" and substring(vc_nodo, 1, "+string(len(s_struttura_loc.vc_nodo))+") = '"+s_struttura_loc.vc_nodo+"'"
	...
	if trap_sql(sqlca, "RUNQUERY01") < 0 then return -1
		...
		do while sqlca.sqlcode <> 100
			...
			li_ret= event ue_queryofferte(ai_qualequery, li_livello_raggruppamento, ls_nodo, s_data_s)
	...
	ll_handle = lv_dettaglio.FindItem(0, directionAll!, false, true, false, false)
		...
		do while ll_handle <> -1
			...
			li_ret= event ue_queryofferte(ai_qualequery, s_struttura_loc.livello, s_struttura_loc.vc_nodo, s_data_s)
...
public function integer lvf_dettaglio ();integer li_ret
...
string picturename[] = {"download01.jpg","download01.jpg","download01.jpg","download01.jpg","download01.jpg","download01.jpg","download01.jpg","download03.jpg"}
