$PBExportHeader$n_gestione_associati_mysql.sru
$PBExportComments$Generated Database Connection Service
forward
global type n_gestione_associati_mysql from nonvisualobject
end type
end forward

global type n_gestione_associati_mysql from nonvisualobject
end type
global n_gestione_associati_mysql n_gestione_associati_mysql
...
Choose Case is_connectfrom
	...
	Case IS_USE_INIFILE						/*  Populate Database Connection from INI file  */
		string ls_inifile = "S:\Svil7\Seg\Gestione_Associati\gestione_associati_mysql.ini"
		...
		as_dbparm		= ProfileString ( ls_inifile, "Database", "DBParm",		"ConnectString='DSN=seg_rm;UID=root;PWD=sysadm',CommitOnDisconnect='No'")
	...
	Case IS_USE_REGISTRY						/*  Populate Database Connection from Registry  */
		...
		If RegistryGet ( ls_registrykey, "DBParm", 		RegString!, as_dbparm ) <> 1 Then
			RegistrySet ( ls_registrykey, "DBParm", 		RegString!, "ConnectString='DSN=seg_rm;UID=root;PWD=sysadm',CommitOnDisconnect='No'" )
	...
	Case IS_USE_SCRIPT							/*  Populate Database Connection from Script  */
		...
		as_logpass		= ""
		as_server		= ""
		as_dbparm		= "ConnectString='DSN=seg_rm;UID=root;PWD=sysadm',CommitOnDisconnect='No'"
		as_lock			= ""
		as_autocommit	= "false"
...
on n_gestione_associati_mysql.create
TriggerEvent( this, "constructor" )
end on

on n_gestione_associati_mysql.destroy
