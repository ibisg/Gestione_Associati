$PBExportHeader$w_struttura_tv_con_lv_ol.srw
forward
global type w_struttura_tv_con_lv_ol from window
end type
type lv_dettaglio from listview within w_struttura_tv_con_lv_ol
end type
type tv_struttura from treeview within w_struttura_tv_con_lv_ol
end type
end forward
...
global w_struttura_tv_con_lv_ol w_struttura_tv_con_lv_ol
...
public function integer lvf_dettaglio (s_struttura as_struttura)
end prototypes
...
do while sqlca.sqlcode <> 100
	...
	w_preview_scheda.dw_current.setItem(ll_row, "intestazione", "Report offerte dal "+as_data_s. ls_da+" al "+as_data_s.ls_a)	
...
if ll_livello <  MAX_LIVELLO then
	...
	do while sqlca.sqlcode <> 100
		...
		ll_lvi.label= f_iniziali(ls_cognome+" "+ls_nome)
...
public function integer lvf_dettaglio (s_struttura as_struttura);long ll_lev1, ll_parent, ll_codice_membro, ll_cod_div, ll_todelete, ll_ret
string ls_relazione, ls_codice, ls_descrizione, ls_cognome, ls_nome, ls_cod_dip
long ll_livello, ll_handle, ll_nodo
listViewItem ll_lvi

ll_nodo= as_struttura.id_nodo
ll_livello= as_struttura.livello
ll_handle= as_struttura.handle
...
//		s_struttura_loc.livello= ll_livello
//		s_struttura_loc.handle= ll_handle
		ll_lvi.data= as_struttura
		
		ll_lvi.label= f_iniziali(ls_descrizione)+"-"+string(ll_handle)
		ll_lvi.pictureIndex= ll_livello
//		ll_lvi.statePictureIndex= 2
		ll_lvi.selected= false
		
		ll_lev1 = lv_dettaglio.addItem(ll_lvi)
...
//		s_struttura_loc.id_nodo= ll_nodo
//		s_struttura_loc.livello= ll_livello
//		s_struttura_loc.handle= as_struttura.handle
...
on w_struttura_tv_con_lv_ol.create
...
on w_struttura_tv_con_lv_ol.destroy
...
lv_dettaglio.x= this.width - 1893 - 30
tv_struttura.width= lv_dettaglio.x - 30
lv_dettaglio.width= this.width - (tv_struttura.width + 30) - 30
...
string picturename[] = {"GLOBES.bmp","a.gif","t.gif","h.gif","c.gif","s.gif","g.gif"}
...
return 0 //tvf_genera_dettaglio(newhandle)
end event
