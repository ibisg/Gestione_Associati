$PBExportHeader$w_esami_tab_old.srw
forward
global type w_esami_tab_old from window
end type
type tab_esami from tab within w_esami_tab_old
end type
type tabpage_inserimento from userobject within tab_esami
end type
type st_criterio4 from statictext within tabpage_inserimento
end type
type st_criterio3 from statictext within tabpage_inserimento
end type
...
type st_criterio2 from statictext within tabpage_inserimento
end type
type st_criterio1 from statictext within tabpage_inserimento
end type
type gb_criteri from groupbox within tabpage_inserimento
end type
type tabpage_inserimento from userobject within tab_esami
st_criterio4 st_criterio4
st_criterio3 st_criterio3
uodw_import uodw_import
st_1 st_1
pb_struttura pb_struttura
cb_importa cb_importa
cb_estrazione cb_estrazione
st_criterio2 st_criterio2
st_criterio1 st_criterio1
...
type dw_header from uodw_header within w_esami_tab_old
end type
type s_cons from structure within w_esami_tab_old
end type
end forward
...
global w_esami_tab_old w_esami_tab_old
...
string ls_descrizione_livello, ls_descrizione_evento
...
select data_ultimo_esame, data_cerimonia_soglia, data_diploma, livello_resp_soglia, a_g
into :idt_data_ultimo_esame, :idt_data_cerimonia_soglia, :idt_data_diploma, :ii_livello_resp_soglia, :is_a_g
from esami_sessioni
...
if isNull(idt_data_ultimo_esame) then
	tab_esami.tabpage_inserimento.st_criterio1.visible= false
else
	tab_esami.tabpage_inserimento.st_criterio1.text= "- possono partecipare coloro che hanno sostenuto l'esame precedente entro il: "+string(idt_data_ultimo_esame, "dd.mm.yyyy")+" (compreso)"
end if

if isNull(idt_data_cerimonia_soglia) then
	tab_esami.tabpage_inserimento.st_criterio2.visible= false
else
	tab_esami.tabpage_inserimento.st_criterio2.text= "- possono partecipare coloro che sono diventati membri entro il: "+string(idt_data_cerimonia_soglia, "dd.mm.yyyy")+" (compreso)"
end if	

if ii_livello_resp_soglia= 0 then
tab_esami.tabpage_inserimento.st_criterio3.visible= false
else
	...
	tab_esami.tabpage_inserimento.st_criterio3.text= "- possono partecipare coloro che hanno almeno una responsabilità di "+ls_descrizione_livello
end if	

choose case is_a_g
		
	case 'A'
		tab_esami.tabpage_inserimento.st_criterio4.text= "- possono partecipare coloro che appartengono ai gruppi adulti"
	case 'G'
		tab_esami.tabpage_inserimento.st_criterio4.text= "- possono partecipare coloro che appartengono ai gruppi giovani"
	case else
...
if is_liv= '00' then
	tab_esami.tabpage_inserimento.uodw_import.dataObject= "dw_import_primo_livello"
else
	tab_esami.tabpage_inserimento.uodw_import.dataObject= "dw_import"	
end if
...
//		ds_lista.SetItem(ll_row, "livello", is_struttura_s.livello)		
		...
		if  tab_esami.tabpage_inserimento.uodw_import.rowCount() > 1000 then
...
this.st_criterio4=create st_criterio4
this.st_criterio3=create st_criterio3
...
this.st_criterio2=create st_criterio2
this.st_criterio1=create st_criterio1
this.gb_criteri=create gb_criteri
this.Control[]={this.st_criterio4,&
this.st_criterio3,&
this.uodw_import,&
this.st_1,&
this.pb_struttura,&
...
destroy(this.st_criterio4)
destroy(this.st_criterio3)
destroy(this.uodw_import)
destroy(this.st_1)
destroy(this.pb_struttura)
destroy(this.cb_importa)
destroy(this.cb_estrazione)
destroy(this.st_criterio2)
destroy(this.st_criterio1)
...
if is_liv= '00' then // Nessun esame...
		...
		ls_sql= "SELECT count(*) FROM membri_gerarchica"+&
					" WHERE membri_gerarchica.cod_studio = '00' "+&
						" and membri_gerarchica.cod_dis in ('MEM', 'TFE') "/*+&
							" and membri_gerarchica.codice NOT IN  (select codice from esami, eventi where esami.id= eventi.id and eventi.tipo_evento= "+string(is_evento_s.tipo_evento)+" )"*/
					
					
			if NOT isNull(idt_data_cerimonia_soglia) then
	
				ls_sql+= " and membri_gerarchica.data_cer <= '"+string(idt_data_cerimonia_soglia, "dd.mm.yyyy")+"'"	
		...
		ls_sql= "SELECT count(*) FROM membri_gerarchica, esami_curr "+&
					" WHERE membri_gerarchica.cod_studio = '"+is_liv+"'"+&
							" and membri_gerarchica.cod_dis in ('MEM', 'TFE') "+&
								"and ( esami_curr.esito = 'P' ) and (esami_curr.cod_studio= "+is_liv+") "/*+ &
									" and membri_gerarchica.codice NOT IN  (select codice from esami, eventi where esami.id= eventi.id and eventi.tipo_evento= "+string(is_evento_s.tipo_evento)+" )"*/
		...
		if NOT isNull(idt_data_ultimo_esame) AND NOT isNull(idt_data_cerimonia_soglia) then
					...
					ls_sql+= " and esami_curr.data_esame <= '"+string(idt_data_ultimo_esame, "dd.mm.yyyy")+"' and membri_gerarchica.data_cer <= '"+string(idt_data_cerimonia_soglia, "dd.mm.yyyy")+"'"
			
		elseif NOT isNull(idt_data_ultimo_esame) AND isNull(idt_data_cerimonia_soglia) then
			
					ls_sql+= " and esami_curr.data_esame <= '"+string(idt_data_ultimo_esame, "dd.mm.yyyy")+"'"
...
ls_where= mid(sqlsyntax, li_pos+5, len(sqlsyntax) )
...
FETCH sql_cur INTO :il_righeestratte ;
...
// Questa è la query modificata con i ceriteri di ricerca....
...
event clicked;if is_struttura_s.livello= 0 and uodw_import.Object.DataWindow.QueryMode = "yes" then
