$PBExportHeader$w_struttura_tv_tab_new.srw
forward
global type w_struttura_tv_tab_new from w_struttura_tv
end type
type tab_dettaglio from tab within w_struttura_tv_tab_new
end type
...
type cb_ricercaanomalie from commandbutton within w_struttura_tv_tab_new
end type
type shl_altritipi from statichyperlink within w_struttura_tv_tab_new
end type
end forward
...
global w_struttura_tv_tab_new w_struttura_tv_tab_new
...
long ll_totale_persone, ll_row, ll_totaleattivi, ll_tai, ll_tfe
...
//			li_ret= w_preview.uodw_current.setItem(ll_row, "intestazione", "Consegne dal "+as_data_s. ls_da+" al "+as_data_s.ls_a)	
			...
			li_ret=w_preview.uodw_current.setItem(ll_row, "totale_tai", ll_tai)		
			li_ret=w_preview.uodw_current.setItem(ll_row, "totale_tfe", ll_tfe)	
...
//					tab_dettaglio.selectTab(1)
			...
			*/	
			...
			tab_dettaglio.tabpage_offerte.visible= 	f_has_p('D')
			
			if is_current_item_s.flag_npa= '1' then
				tab_dettaglio.tabpage_liste.visible= false
			else
				tab_dettaglio.tabpage_liste.visible= 	( f_has_p('S') OR s_utente_s.s_invioliste= 'S')
			end if
...
public function integer wf_handle_rb ();string okatagi, tokuso, omamori

		tab_dettaglio.tabpage_goh.em_luogo.text= ""
		tab_dettaglio.tabpage_goh.em_data.text= string(today(), "dd.mm.yyyy")
			
		select		
			dbo.uf_situazione_okatagi(membri_gerarchica.codice, membri_gerarchica.cod_dis), 
			dbo.uf_situazione_tokuso(membri_gerarchica.codice, membri_gerarchica.cod_dis),
...
on w_struttura_tv_tab_new.create
int iCurrent
call super::create
...
on w_struttura_tv_tab_new.destroy
call super::destroy
...
type phl_trova from w_struttura_tv`phl_trova within w_struttura_tv_tab_new
boolean map3dcolors = false
string powertiptext = "Esegue la ricerca per nome o struttura"
end type

type st_ricerca1 from w_struttura_tv`st_ricerca1 within w_struttura_tv_tab_new
integer width = 658
end type

type dw_cerca from w_struttura_tv`dw_cerca within w_struttura_tv_tab_new
integer taborder = 10
end type

type tv_struttura from w_struttura_tv`tv_struttura within w_struttura_tv_tab_new
...
//		
	...
	case 3
		...
		if NOT isNull(tab_dettaglio.tabpage_base.uodw_aggiornamento.getRow()) then
...
event type integer ue_struttura();		integer li_ret
		...
		for ll_idx= 1 to this.totalItems()
			...
			if  s_struttura_s.flag_goh= 'S' OR s_struttura_s.flag_tok= 'S' then
...
boolean bringtotop = true
string dataobject = "dw_anagrafica_test"
...
//		end if
	...
	case "b_struttura"
		...
		if  this.getItemString(row, "flag_goh") = 'S' OR  this.getItemString(row, "flag_tok") = 'S' then
		...
		case "cod_dis" // Status può essere: MEM, DIM, DEC, TFE, ISG, NOP
						...
						if data= "NOP" then
							...
							if ls_codice_fam <> "" and NOT isNull(ls_codice_fam) then
								...
								if f_disabilitaaccessoremoto(ls_session_s.codice) = -1 then goto erroresql	
...
// Se l'utente ha il ruolo supervisore per una certa funzione o Direzione Segreteria...
 ...
 if ( (s_utente_s.i_ruolo_stat= 4 OR s_utente_s.i_ruolo_rga= 2 OR s_utente_s.i_ruolo_opc= 1 OR s_utente_s.i_ruolo_ediz= 5 OR f_has_p('S'))/*  AND  this.getItemString(1, "cod_dis")= 'MEM' AND this.getItemString(1, "flag_npa")= '0'*/) then
...
if upperBound(s_consegna_s)= 1 AND mid(s_consegna_s[1].causale, 1, 1)= 'R'  then
...
//		if s_consegna_s[ll_row].causale= 'ACQ' or s_consegna_s[ll_row].causale= 'RSO' then
...
//			if  s_consegna_s[ll_row].causale= 'ACQ' then
//				
//				ls_session_s.gm= 'G'
//				
//				ls_session_s.tipo_movimento_src= "A-ALT"	
//				
//			else
//				
//				ls_session_s.gm= 'M'
//				
//				ls_session_s.tipo_movimento_src= "D-ALT"		
//				
//			end if
