$PBExportHeader$w_ins_eventi_old.srw
forward
global type w_ins_eventi_old from window
end type
type st_1 from statictext within w_ins_eventi_old
end type
type cb_chiudi from commandbutton within w_ins_eventi_old
end type
type uodw_ins_eventi from uodw_generica within w_ins_eventi_old
end type
end forward
...
global w_ins_eventi_old w_ins_eventi_old
...
case "tipo_evento"
	...
	if integer(data)= 6 then // cerimonie di entrata
		...
		if integer(data) < 6 then
			...
			// verifica se esiste una sessione d'esame al livello selezionato
			...
			this.setItem(row, "id_esami_sessioni", idx)			
			...
			this.setItem(row, "operatore", s_utente_s.s_nome_esteso)	
