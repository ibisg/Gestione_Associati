$PBExportHeader$uotv_struttura_old.sru
forward
global type uotv_struttura_old from treeview
end type
end forward
...
string picturename[] = {"wi0064-48.gif","foot_orange.gif","foot_blue.gif","foot_red.gif","foot_yellow.gif","foot_green.gif","foot_orange.gif","CharlieBrown2.gif","lucy_32x32.jpg","Linus.gif","Sally.gif","bullet_square_yellow.gif","bullet_square_yellow.gif",""}
...
global uotv_struttura_old uotv_struttura_old
...
integer li_ret
long ll_handle
string ls_token, ls_match, ls_nodo
...
as_struttura= mid(as_struttura, len(is_tvinit_s.as_ambito) + 2, len(as_struttura) )
...
if as_struttura= "" then // se è blank vuol dire che sono già sul punto giusto
			...
			do while NOT lb_found
				...
				ls_nodo= is_tvinit_s.as_ambito+"."+ls_token
...
//		from struttura
//		where vc_nodo = :is_tvinit_s.as_ambito;
...
//if is_struttura_s.livello < is_tvinit_s.ai_maxlivello then

if s_struttura_root_s.livello < is_tvinit_s.ai_maxlivello then

	ll_rows= ids_populate.retrieve(s_struttura_root_s.vc_nodo, is_tvinit_s.as_ambito)
	
	if trap_sql(sqlca, "LIV0") < 0 then goto errore
	
	for ll_idx= 1 to ll_rows
		
		s_struttura_child_s.livello= ids_populate.getItemNumber(ll_idx, "struttura_livello")

		s_struttura_child_s.vc_nodo= ids_populate.getItemString(ll_idx, "struttura_vc_nodo")
		
		s_struttura_child_s.relazione= ids_populate.getItemString(ll_idx, "struttura_relazione")
		
		s_struttura_child_s.vc_parent= ids_populate.getItemString(ll_idx, "struttura_vc_parent")
		
		s_struttura_child_s.codice= ids_populate.getItemString(ll_idx, "struttura_codice")
		
		s_struttura_child_s.descrizione_livello= ids_populate.getItemString(ll_idx, "struttura_livelli_descrizione")
		
		s_struttura_child_s.descrizione_struttura= ids_populate.getItemString(ll_idx, "struttura_descrizione")
...
//			end if
	...
	next
			...
			ll_rows= ids_membri.retrieve(s_struttura_root_s.vc_nodo)
			...
			for ll_idx= 1 to ll_rows
				...
				s_struttura_child_s.codice=	ids_membri.getItemString(ll_idx, "codice")
				s_struttura_child_s.cognome=	ids_membri.getItemString(ll_idx, "cognome")
				s_struttura_child_s.nome=	ids_membri.getItemString(ll_idx, "nome")
				s_struttura_child_s.cod_div=	integer(ids_membri.getItemString(ll_idx, "cod_div"))
				s_struttura_child_s.divisione=	ids_membri.getItemString(ll_idx, "divisione")
				s_struttura_child_s.citta_nas=	ids_membri.getItemString(ll_idx, "citta_nas")
				s_struttura_child_s.prov_nas=	ids_membri.getItemString(ll_idx, "prov_nas")
				s_struttura_child_s.data_nas=	ids_membri.getItemDatetime(ll_idx, "data_nas")
				s_struttura_child_s.ind_dom=	ids_membri.getItemString(ll_idx, "ind_dom")
				s_struttura_child_s.cap_dom=	ids_membri.getItemString(ll_idx, "cap_dom")
				s_struttura_child_s.comune=	ids_membri.getItemString(ll_idx, "comune")
				s_struttura_child_s.provincia=	ids_membri.getItemString(ll_idx, "provincia")
				s_struttura_child_s.professione=	ids_membri.getItemString(ll_idx, "professione")
				s_struttura_child_s.pres_da=	ids_membri.getItemString(ll_idx, "pres_da")
				s_struttura_child_s.data_cer=	ids_membri.getItemDatetime(ll_idx, "data_cer")
				s_struttura_child_s.luogo_cer=	ids_membri.getItemString(ll_idx, "luogo_cer")
				s_struttura_child_s.responsabilita1=	ids_membri.getItemString(ll_idx, "resp1")
				s_struttura_child_s.responsabilita2=	ids_membri.getItemString(ll_idx, "resp2")
				s_struttura_child_s.livello_studio=	ids_membri.getItemString(ll_idx, "livello_studio")
				s_struttura_child_s.flag_goh=	ids_membri.getItemString(ll_idx, "flag_goh")
				s_struttura_child_s.flag_tok=	ids_membri.getItemString(ll_idx, "flag_tok")
				s_struttura_child_s.flag_oma=	ids_membri.getItemString(ll_idx, "flag_oma")
				s_struttura_child_s.flag_er=	ids_membri.getItemString(ll_idx, "flag_er")
				s_struttura_child_s.status=	ids_membri.getItemString(ll_idx, "status")
				s_struttura_child_s.cod_dis=	ids_membri.getItemString(ll_idx, "cod_dis")
				s_struttura_child_s.flag_ldr=	ids_membri.getItemString(ll_idx, "flag_ldr")
				s_struttura_child_s.flag_npa=	ids_membri.getItemString(ll_idx, "flag_npa")	
...
on uotv_struttura_old.create
end on

on uotv_struttura_old.destroy
end on
...
ids_populate.DataObject = 'ds_populatemulti'
